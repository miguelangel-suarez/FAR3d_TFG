...
c        write(*,*) ns,mn_col,noeqn
        allocate(im_col(mn_col), in_col(mn_col), stat=ierr)
        allocate(rho(ns), stat=ierr)
        do i=1,mn_col
         read(15,*) im_col(i), in_col(i)
...
c  be modified for other uses.
       do j=1,kmax
        do m = 1,ns
         do i = 1,mn_col/2
          mneg = mn_col - i + 1
c          write(33,*) sqrt(egn_vectors(j,i,2,m)**2
c     >          + egn_vectors(j,mneg,2,m)**2)
          write(33,*) egn_vectors(j,i,2,m)
          write(33,*) egn_vectors(j,mneg,2,m)
          end do
         end do
       end do
       close(unit=33)
