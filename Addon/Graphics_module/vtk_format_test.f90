...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	module var_para
		...
		implicit none
		save
		integer :: nr_start,nr_end,nc_start,nc_end
		integer :: mj_start,mj_end
		integer, dimension(:), allocatable :: n_st,n_nd,nr_st,nr_nd,nc_st,nc_nd,lmaxPE
	...
	module cotrol
		...
		integer :: ihist,nocpl,maxstp,nstep,ndump,nprint,lplots,itime,nstep1,nonlin,noeqn,edge_p, &
		           ...
		           EP_dens_on,Alpha_dens_on,EP_vel_on,Alpha_vel_on,Trapped_on,ext_prof_len,q_prof_on,Eq_vel_on, & 
                           ...
                           pr_vtk,psi_vtk,phi_vtk,uzt_vtk,nf_vtk,vprlf_vtk,vthprlf_vtk, &
                           vr_vtk,vth_vtk,Br_vtk,Bth_vtk,nalp_vtk,vprlalp_vtk,Vvec_vtk,Bvec_vtk, &
                           preq_vtk,nfeq_vtk,ifreezd,ifreezu,jint,jmn,jmx,ndiag,numeq
		integer :: nx_vtk,ny_vtk,nz_vtk,mj_r,lth_r,lzt_r
                                                                         
		character(len=40) :: eq_name,ext_prof_name,name_vtk		
		logical :: matrix_out
	...
	module domain
		...
		real(IDP), dimension(:), allocatable :: dnnbi,dne,dni,temp_epnn,ti,te,temp_ep,qprofile, &
							...
							dnalpha,dnalphann,talpha,talphann,vzt_eqp,vth_eqp,vtherm_elcP,vtherm_elc
	...
	module equil
		...
		real(IDP), dimension(:), allocatable :: qq,qqinv,qqinvp,denseq,denseqr,psieq,chieq,preq,feq,cureq,teeq,nfeq,dnfeqdr,vfova, &
							vfova2,vtherm_ion,vzt_eq,vth_eq,vth_eq1,nalpeq,valphaova,valphaova2,dnalpeqdr, &
                                                        preq_r,nfeq_r,qqinv_r,tieq
		character(len=8), dimension(2) :: ndevice
		integer :: ngeneq,leq,ndat,ltwo,ieq,nqprof,ntok
		real(IDP), dimension(100) :: xr,xdat,pdat
		real(IDP) :: eps,bet0,pop,popp,pol2,betaj,betat,betap,q0,qlamb,ro,qh,qlambpp,rsbar,xle
	...
	module dynamo
		...
		implicit none
		save
		integer :: ietaeq,ipert,neta,ith,izt,ichipr
		real(IDP), dimension(100) :: etadat
		real(IDP) :: dt,dtd2,time,s,gamma,pertscl,chipr,rchi		
		...
		real(IDP), dimension(:,:,:), allocatable :: psi_b,phi_b,pr_b,uzt_b,nf_b,vprlf_b, &
                                           	vthprlf_b,Vr_b,Vth_b,Br_b,Bth_b,Bzt_b,sqg_b,sqgi_b, &
                                                nalp_b,vprlalp_b,rmn_c,zmn_c,pmn_c,xx1,xx2,xx3, &
                                                preq_b,nfeq_b,Breq_b,Btheq_b,Bzteq_b		
	end module dynamo
...
!		Read the input files
                ...
                open (unit=6,file="Input_GM",status="old",form="formatted")
...
20    rewind(5)		
		...
		allocate (dnalpeqdr(0:jdim))	!	Radial derivate of the normalized density of the second fast particle species
	        allocate (vAlfven(0:jdim))
	        allocate (vtherm_elcP(0:jdim))
	        allocate (vtherm_elc(0:jdim))
		...
		allocate (sqg_b(0:mj_r,0:lth_r,0:lzt_r))
		...
		allocate (preq_b(0:mj_r,0:lth_r,0:lzt_r))
		allocate (nfeq_b(0:mj_r,0:lth_r,0:lzt_r))
		allocate (qqinv_r(0:mj_r))
		allocate (Breq_b(0:mj_r,0:lth_r,0:lzt_r))	
		allocate (Btheq_b(0:mj_r,0:lth_r,0:lzt_r))	
		allocate (Bzteq_b(0:mj_r,0:lth_r,0:lzt_r))	
...
!		Default values of the model variables and parameters						
	...
	subroutine setmod
		...
		integer :: l,n,m,lh,mt,nt,ngcd,ndiv,lll,i,lm,lq,mcnt,nl,lp,lpp,lppn,lop,l1,mband,mmp,nnp,np,idel,idelp1,i_mn,ip,mjst,mjnd, &
			   iPE,mxnum,neq,nm
		integer, dimension(1) :: PE_min
		integer, dimension(:), allocatable :: nsort,nmap,nnum_PE,nrsq_PE,nrl_PE,nvalue,iflag
...
!			write(6,'(3i5)') (lh,mh(lh),nh(lh),lh=1,lhmax)
		...
		if (nmaxeq > 0) then
			do l=1,leqmax
				if (nneq(l) > 0) nfp=min(nfp,nneq(l))
			end do
			do l=1,leqmax
				if (nneq(l) /= nfp*(nneq(l)/nfp)) then
					write (6,'("  setmod: nneq(",i2,") =",i4," is not multiple of nfp =",i3)') l,nneq(l),nfp
					stop
				end if
			end do
		else
			...
			do np=1,nnum
				...
				if (lpp > 0) then
					...
					if (nmin == 0 .or. nmax == 0 .or. n == 0) then
						...
						if (myPE == 0) write (6,'("  setmod: n =",i4," mnumn =",i3,i4)') n,lpp,lppn
						stop
					end if
				else
					nl=nl-1
...
!		The data of the previous run is read to continue the simulation		
		...
		confil=name_vtk		
		open(unit=8,file=confil,status='unknown',convert='big_endian',form='unformatted')
		allocate (numhist(1:100))

		read(8) ihisto,numruno,numrunp,numruns,numeq,numvac,(numhist(i),i=1,ihisto), maxstp,nstep, &
                        ndump,nprint,lplots,ndiag,jmn,jmx,jint,itime,dt0,nstep1,nonlin,m0dy,nocpl,ifreezd,ifreezu, &
		...
		read(8) mj,lmax,leqmax,mjm1
		read(8) (r(j),j=0,mj),(mm(l),l=1,lmax),(nn(l),l=1,lmax),(mmeq(l),l=1,leqmax),(nneq(l),l=1,leqmax),(rinv(j),j=0,mj), &		
		...
		read(8) (qq(j),j=0,mj),(qqinv(j),j=0,mj),(psieq(j),j=0,mj),(chieq(j),j=0,mj),(preq(j),j=0,mj),(feq(j),j=0,mj), &
			(cureq(j),j=0,mj),(denseq(j),j=0,mj),(teeq(j),j=0,mj),(nfeq(j),j=0,mj),(vfova(j),j=0,mj),(vzt_eq(j),j=0,mj),(tieq(j),j=0,mj)
		if (alpha_on == 1) read(8) (nalpeq(j),j=0,mj),(valphaova(j),j=0,mj)

		if (ext_prof == 1) then
			read(8) (vAlfven(j),j=0,mj),(vtherm_elcP(j),j=0,mj)
		else
			read(8) (vtherm_elc(j),j=0,mj)
		end if
                
		read(8) ngeneq,ndevice(1),ndevice(2),ieq,eps,bet0,bet0_f,nqprof,q0,qlamb,ro,(rs(l),l=1,lmax),leq,qh,rsbar,qlambpp, &
			ndat,(xr(j),j=1,ndat),(xdat(j),j=1,ndat),pop,popp,pol2,ltwo,betaj,(pdat(j),j=1,ndat),ntok,betat,betap
		...
		read(8) etascl,reta,eta0,etalmb,ietaeq,(etadat(j),j=1,ndat),chipr,ichipr,rchi,stdifp,stdifu,stdifnf,stdifv,stdifvf, &
	...
	subroutine vmec
		...
		integer :: j,k,l,l1,lq,mjeqp,mjeqm1,mpol,ntor,lb0,m,m2,mm1,mm2,mm12,mm22,mm2p1,mm2p12,mp2,mp22,isg,idummy,mband,n,nnn,mmm,lbm2, &
...
!               Check input rmnb, zmnb, pmnb

!		write(confil,'("test_rmnb")')
!		open(unit=201,file=confil,recl=2048)
!		write(201,formatt) (t,mmb(l),nnb(l),l=1,lbmax)
!		do j=1,mjeq
!			write(201,formatv) rfar(j),(t,rmnb(j,l),l=1,lbmax)
!		end do
!		close(201)
!		write(confil,'("test2_zmnb")')
!		open(unit=202,file=confil,recl=2048)
!		write(202,formatt) (t,mmb(l),nnb(l),l=1,lbmax)
!		do j=1,mjeq
!			write(202,formatv) rfar(j),(t,zmnb(j,l),l=1,lbmax)
!		end do
!		close(202)
!		write(confil,'("test3_pmnb")')
!		open(unit=203,file=confil,recl=2048)
!		write(203,formatt) (t,mmb(l),nnb(l),l=1,lbmax)
!		do j=1,mjeq
!			write(203,formatv) rfar(j),(t,pmnb(j,l),l=1,lbmax)
!		end do
!		close(203)
...
!		write(0,'("m_sel = ",i4)') m_sel
!		write(0,'("ltop = ",i4)') ltop

		ir=max(15,mjeq/10)
		do l=1,ltop
			m=abs(mmb(l))
			m2=m
!			call eqsplns(rmneq(:,l),sd1_r,rmnb(:,l),m,m2,"icsscu",1.e-07_IDP)
!			call eqsplns(zmneq(:,l),sd1_r,zmnb(:,l),m,m2,"icsscu",1.e-07_IDP)
!			call eqsplns(pmneq(:,l),sd1_r,pmnb(:,l),m,m2,"icsscu",1.e-07_IDP)
			mm1=abs(m-1)
			mm2=abs(mm1-1)
			mm12=mm1
			mm22=mm2
			...
			mp2=m+2
			mp22=mp2
			call fitter(sd2_r,sd1_r,rmnb(:,l),ir,m)
                        rmneq(:,l) = sd2_r(:)
			call fitter(sd2_r,sd1_r,zmnb(:,l),ir,m)
                        zmneq(:,l) = sd2_r(:)
			call fitter(sd2_r,sd1_r,pmnb(:,l),ir,m)
                        pmneq(:,l) = sd2_r(:)
		end do              
...
!               Check splines rmneq, zmneq, pmneq

!		write(confil,'("test_rmneq")')
!		open(unit=200,file=confil,recl=2048)
!		write(200,formatt) (t,mmb(l),nnb(l),l=1,lbm2)
!		do j=1,mj_r
!			write(200,formatv) r_red(j),(t,rmneq(j,l),l=1,lbm2)
!		end do
!		close(200)
!		write(confil,'("test_zmneq")')
!		open(unit=201,file=confil,recl=2048)
!		write(201,formatt) (t,mmb(l),nnb(l),l=1,lbm2)
!		do j=1,mj_r
!			write(201,formatv) r_red(j),(t,zmneq(j,l),l=1,lbm2)
!		end do
!		close(201)
!		write(confil,'("test_pmneq")')
!		open(unit=202,file=confil,recl=2048)
!		write(202,formatt) (t,mmb(l),nnb(l),l=1,lbm2)
!		do j=1,mj_r
!			write(202,formatv) r_red(j),(t,pmneq(j,l),l=1,lbm2)
!		end do
!		close(202)
...
!	neta is the least common multiple of lth_r and lzt_r
	...
	subroutine sum_var
		...
		real(IDP), dimension(:,:), allocatable :: psi_r,phi_r,pr_r,uzt_r,nf_r,vprlf_r,vthprlf_r, &
                                                          Vr_r,Vth_r,Br_r,Bth_r,sqg_r,sqgi_r,nalp_r,vprlalp_r
		...
		interface
			...
			subroutine fitter2(yeq,yeqr,yfar,ir,m)
				use param
				implicit none
				integer :: ir,m
				real(IDP), dimension(0:) :: yeq,yeqr,yfar
			end subroutine fitter2	
		end interface
...
!  bth up
		...
		allocate (psi_r(0:mj_r,0:ldim),phi_r(0:mj_r,0:ldim),pr_r(0:mj_r,0:ldim),uzt_r(0:mj_r,0:ldim), &
                          ...
                          Vth_r(0:mj_r,0:ldim),Br_r(0:mj_r,0:ldim),Bth_r(0:mj_r,0:ldim),sqg_r(0:mj_r,0:ldim), &
                          sqgi_r(0:mj_r,0:ldim),nalp_r(0:mj_r,0:ldim),vprlalp_r(0:mj_r,0:ldim))

		do l=1,lmaxn
			m=abs(mm(l))
			m2=m
			call splns(psi_r(:,l),sd1,psi(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(phi_r(:,l),sd1,phi(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(pr_r(:,l),sd1,pr(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(uzt_r(:,l),sd1,uzt(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(nf_r(:,l),sd1,nf(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(vprlf_r(:,l),sd1,vprlf(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(vthprlf_r(:,l),sd1,vthprlf(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(Vr_r(:,l),sd1,sc1(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(Vth_r(:,l),sd1,sc2(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(Br_r(:,l),sd1,sc5(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(Bth_r(:,l),sd1,sc6(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(sqg_r(:,l),sd1,sqg(:,l),m,m2,"spline",1.e-07_IDP)
			call splns(sqgi_r(:,l),sd1,sqgi(:,l),m,m2,"spline",1.e-07_IDP)
                       if(alpha_on .eq. 1) then
			    call splns(nalp_r(:,l),sd1,nalp(:,l),m,m2,"spline",1.e-07_IDP)
			    call splns(vprlalp_r(:,l),sd1,vprlalp(:,l),m,m2,"spline",1.e-07_IDP)
                        end if
		end do

!               Equilibrium variables interpolations

                allocate (preq_r(0:mj_r),nfeq_r(0:mj_r))
                sc1(:,0) = preq(:)
                call splns(preq_r(:),sd1,sc1(:,0),0,0,"spline",1.e-07_IDP)
                sc1(:,0) = nfeq(:)
                call splns(nfeq_r(:),sd1,sc1(:,0),0,0,"spline",1.e-07_IDP)
                sc1(:,0) = qqinv(:)
                call splns(qqinv_r(:),sd1,sc1(:,0),0,0,"spline",1.e-07_IDP)

!		write(confil,'("test_qqinv")')
!		open(unit=200,file=confil,recl=2048)
!		do j=1,mj
!			write(200,'(1pe13.6,1pe13.6)') r(j),qqinv(j)
!		end do
!		close(200)
!		write(confil,'("test_qqinv_r")')
!		open(unit=201,file=confil,recl=2048)
!		do j=1,mj_r
!			write(201,'(1pe13.6,1pe13.6)') r_red(j),qqinv_r(j)
!		end do
!		close(201)
			...
			do lzt = 0,lzt_r
				...
				do l=1,lmaxn
					if (signl(lln(l)) == 0) then  !(0,0) part
						psi_b(j,lth,lzt)=psi_b(j,lth,lzt)+psi_r(j,lln(l))
						phi_b(j,lth,lzt)=phi_b(j,lth,lzt)+phi_r(j,lln(l))
						pr_b(j,lth,lzt)=pr_b(j,lth,lzt)+pr_r(j,lln(l))
						uzt_b(j,lth,lzt)=uzt_b(j,lth,lzt)+uzt_r(j,lln(l))
						nf_b(j,lth,lzt)=nf_b(j,lth,lzt)+nf_r(j,lln(l))
						vprlf_b(j,lth,lzt)=vprlf_b(j,lth,lzt)+vprlf_r(j,lln(l))
						vthprlf_b(j,lth,lzt)=vthprlf_b(j,lth,lzt)+vthprlf_r(j,lln(l))
						Vr_b(j,lth,lzt)=Vr_b(j,lth,lzt)+Vr_r(j,lln(l))
						Vth_b(j,lth,lzt)=Vth_b(j,lth,lzt)+Vth_r(j,lln(l))
						Br_b(j,lth,lzt)=Br_b(j,lth,lzt)+Br_r(j,lln(l))
						Bth_b(j,lth,lzt)=Bth_b(j,lth,lzt)+Bth_r(j,lln(l))
						sqg_b(j,lth,lzt)=sqg_b(j,lth,lzt)+sqg_r(j,lln(l))
						sqgi_b(j,lth,lzt)=sqgi_b(j,lth,lzt)+sqgi_r(j,lln(l))
						if(alpha_on .eq. 1) then
							nalp_b(j,lth,lzt)=nalp_b(j,lth,lzt)+nalp_r(j,lln(l))
							vprlalp_b(j,lth,lzt)=vprlalp_b(j,lth,lzt)+vprlalp_r(j,lln(l))
						end if                                       
					else if (signl(lln(l)) > 0) then  !sin part
						ithzt=modulo(mm(lln(l))*ith*lth+nn(lln(l))*izt*lzt,neta)
						psi_b(j,lth,lzt)=psi_b(j,lth,lzt)+psi_r(j,lln(l))*cs(ithzt)
						phi_b(j,lth,lzt)=phi_b(j,lth,lzt)+phi_r(j,lln(l))*sn(ithzt)
						pr_b(j,lth,lzt)=pr_b(j,lth,lzt)+pr_r(j,lln(l))*cs(ithzt)
						uzt_b(j,lth,lzt)=uzt_b(j,lth,lzt)+uzt_r(j,lln(l))*sn(ithzt)
						nf_b(j,lth,lzt)=nf_b(j,lth,lzt)+nf_r(j,lln(l))*cs(ithzt)
						vprlf_b(j,lth,lzt)=vprlf_b(j,lth,lzt)+vprlf_r(j,lln(l))*sn(ithzt)
						vthprlf_b(j,lth,lzt)=vthprlf_b(j,lth,lzt)+vthprlf_r(j,lln(l))*sn(ithzt)
						Vr_b(j,lth,lzt)=Vr_b(j,lth,lzt)+Vr_r(j,lln(l))*cs(ithzt)
						Vth_b(j,lth,lzt)=Vth_b(j,lth,lzt)+Vth_r(j,lln(l))*sn(ithzt)
						Br_b(j,lth,lzt)=Br_b(j,lth,lzt)+Br_r(j,lln(l))*sn(ithzt)
						Bth_b(j,lth,lzt)=Bth_b(j,lth,lzt)+Bth_r(j,lln(l))*cs(ithzt)
						sqg_b(j,lth,lzt)=sqg_b(j,lth,lzt)+sqg_r(j,lln(l))*cs(ithzt)
						sqgi_b(j,lth,lzt)=sqgi_b(j,lth,lzt)+sqgi_r(j,lln(l))*cs(ithzt)
						if(alpha_on .eq. 1) then
							nalp_b(j,lth,lzt)=nalp_b(j,lth,lzt)+nalp_r(j,lln(l))*cs(ithzt)
							vprlalp_b(j,lth,lzt)=vprlalp_b(j,lth,lzt)+vprlalp_r(j,lln(l))*sn(ithzt)
						end if
					else  !cos part
						ithzt=modulo(-mm(lln(l))*ith*lth-nn(lln(l))*izt*lzt,neta)
						psi_b(j,lth,lzt)=psi_b(j,lth,lzt)+psi_r(j,lln(l))*sn(ithzt)
						phi_b(j,lth,lzt)=phi_b(j,lth,lzt)+phi_r(j,lln(l))*cs(ithzt)
						pr_b(j,lth,lzt)=pr_b(j,lth,lzt)+pr_r(j,lln(l))*sn(ithzt)
						uzt_b(j,lth,lzt)=uzt_b(j,lth,lzt)+uzt_r(j,lln(l))*cs(ithzt)
						nf_b(j,lth,lzt)=nf_b(j,lth,lzt)+nf_r(j,lln(l))*sn(ithzt)
						vprlf_b(j,lth,lzt)=vprlf_b(j,lth,lzt)+vprlf_r(j,lln(l))*cs(ithzt)
						vthprlf_b(j,lth,lzt)=vthprlf_b(j,lth,lzt)+vthprlf_r(j,lln(l))*cs(ithzt)
						Vr_b(j,lth,lzt)=Vr_b(j,lth,lzt)+Vr_r(j,lln(l))*sn(ithzt)
						Vth_b(j,lth,lzt)=Vth_b(j,lth,lzt)+Vth_r(j,lln(l))*cs(ithzt)
						Br_b(j,lth,lzt)=Br_b(j,lth,lzt)+Br_r(j,lln(l))*cs(ithzt)
						Bth_b(j,lth,lzt)=Bth_b(j,lth,lzt)+Bth_r(j,lln(l))*sn(ithzt)
						sqg_b(j,lth,lzt)=sqg_b(j,lth,lzt)+sqg_r(j,lln(l))*sn(ithzt)
						sqgi_b(j,lth,lzt)=sqgi_b(j,lth,lzt)+sqgi_r(j,lln(l))*sn(ithzt)
						if(alpha_on .eq. 1) then
							nalp_b(j,lth,lzt)=nalp_b(j,lth,lzt)+nalp_r(j,lln(l))*sn(ithzt)
							vprlalp_b(j,lth,lzt)=vprlalp_b(j,lth,lzt)+vprlalp_r(j,lln(l))*cs(ithzt)
						end if
					end if
				end do 
                            end do
			 ...
			 do lth = 0,lth_r
			    do lzt = 0,lzt_r
                                if (abs(psi_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     psi_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(phi_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     phi_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(pr_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     pr_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(uzt_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     uzt_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(nf_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     nf_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(vprlf_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     vprlf_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(vthprlf_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     vthprlf_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(Vr_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     Vr_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(Vth_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     Vth_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(Br_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     Br_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(Bth_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     Bth_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(sqg_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     sqg_b(j,lth,lzt) = 0.0_IDP
                                end if
                                if (abs(sqgi_b(j,lth,lzt)) < 1.e-20_IDP) then  
                                     sqgi_b(j,lth,lzt) = 0.0_IDP
                                end if
...
!       Variables equilibrium + perturbation

                do j = 0,mj_r
			 do lth = 0,lth_r
			    do lzt = 0,lzt_r
		                preq_b(j,lth,lzt)=pr_b(j,lth,lzt)+preq_r(j)   
		            end do
		        end do
                end do     
			 ...
			 do lth = 0,lth_r
			    do lzt = 0,lzt_r
		                nfeq_b(j,lth,lzt)=nf_b(j,lth,lzt)+nfeq_r(j)  
		            end do
		        end do
                end do
			 ...
			 do lth = 0,lth_r
			    do lzt = 0,lzt_r
		                Breq_b(j,lth,lzt)=Br_b(j,lth,lzt)/s
		                Btheq_b(j,lth,lzt)=Bth_b(j,lth,lzt)/s - (eps*r_red(j)*qqinv_r(j)*sqgi_b(j,lth,lzt))  
		                Bzteq_b(j,lth,lzt)=sqgi_b(j,lth,lzt)
		            end do
		        end do
                end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(100,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
            ...
            if(psi_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("psi_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=101,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(101,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(101,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(101,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(101,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(101,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(101,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(101,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(101,'("SCALARS psi float")')
		write(101,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(101,'(1pe15.8)') psi_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(phi_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("phi_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=102,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(102,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(102,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(102,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(102,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(102,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(102,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(102,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(102,'("SCALARS phi float")')
		write(102,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(102,'(1pe15.8)') phi_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(uzt_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("uzt_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=103,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(103,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(103,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(103,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(103,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(103,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(103,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(103,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(103,'("SCALARS uzt float")')
		write(103,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(103,'(1pe15.8)') uzt_b(j,lth,lzt)
		        end do
		    end do
		end do
...
!!!!!!!!!!!!!!  Open the file
		...
		write(confil,'("nf_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=104,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(104,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(104,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(104,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(104,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(104,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(104,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(104,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(104,'("SCALARS nf float")')
		write(104,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(104,'(1pe15.8)') nf_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(vprlf_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("vprlf_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=105,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(105,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(105,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(105,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(105,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(105,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(105,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(105,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(105,'("SCALARS vprlf float")')
		write(105,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(105,'(1pe15.8)') vprlf_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(vthprlf_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("vthprlf_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=106,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(106,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(106,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(106,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(106,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(106,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(106,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(106,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(106,'("SCALARS vthprlf float")')
		write(106,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(106,'(1pe15.8)') vthprlf_b(j,lth,lzt)
		        end do
		    end do
		end do
...
!!!!!!!!!!!!!!  Open the file
		...
		write(confil,'("vr_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=107,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(107,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(107,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(107,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(107,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(107,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(107,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(107,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(107,'("SCALARS vr float")')
		write(107,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(107,'(1pe15.8)') vr_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(vth_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("vth_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=108,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(108,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(108,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(108,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(108,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(108,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(108,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(108,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(108,'("SCALARS vth float")')
		write(108,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(108,'(1pe15.8)') vth_b(j,lth,lzt)
		        end do
		    end do
		end do
...
!!!!!!!!!!!!!!  Open the file
		...
		write(confil,'("Br_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=109,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(109,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(109,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(109,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(109,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(109,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(109,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(109,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(109,'("SCALARS Br float")')
		write(109,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(109,'(1pe15.8)') Br_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(Bth_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("Bth_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=110,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(110,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(110,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(110,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(110,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(110,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(110,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(110,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(110,'("SCALARS Bth float")')
		write(110,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(110,'(1pe15.8)') Bth_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(nalp_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("nalp_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=111,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(111,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(111,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(111,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(111,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(111,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(111,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(111,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(111,'("SCALARS nalp float")')
		write(111,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(111,'(1pe15.8)') nalp_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(vprlalp_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("vprlalp_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=112,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(112,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(112,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(112,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(112,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(112,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(112,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(112,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(112,'("SCALARS vprlalp float")')
		write(112,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(112,'(1pe15.8)') vprlalp_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(preq_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("prq_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=113,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(113,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(113,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(113,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(113,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(113,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(113,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(113,'("SCALARS preq float")')
		write(113,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(113,'(1pe15.8)') preq_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(nfeq_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("nfq_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=114,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(114,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(114,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(114,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(114,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(114,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(114,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(114,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write scalar data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(114,'("SCALARS nfeq float")')
		write(114,'("LOOKUP_TABLE default")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(114,'(1pe15.8)') nfeq_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(Bvec_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("Bv_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=115,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(115,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(115,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(115,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(115,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(115,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(115,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(115,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write vector data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(115,'("VECTORS Bvec float")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(115,'(1pe15.8," ",1pe15.8," ",1pe15.8)') Breq_b(j,lth,lzt),Btheq_b(j,lth,lzt),Bzteq_b(j,lth,lzt)
		        end do
		    end do
		end do
            ...
            if(Vvec_vtk .eq. 1) then

!!!!!!!!!!!!!!  Open the file

		write(confil,'("Vv_",2a2,".vtk")') numrun(1),numrun(2)
		open(unit=116,file=confil,status='new',recl=2048)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
!!!!!!!!!!!!!   1. File version and identifier
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

		write(116,'("# vtk DataFile Version 2.0")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   2. Header
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(116,'("FAR3d VTK Data")')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		write(116,'("DATASET STRUCTURED_GRID")')

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   5. Mesh dimensions
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(116,'("DIMENSIONS ",i4," ",i4," ",i4)') nx1,nx2,nx3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                points = nx1*nx2*nx3
		write(116,'("POINTS ",i10," float")') points
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			    write(116,'(1pe15.8," ",1pe15.8," ",1pe15.8)') xx1(j,lth,lzt),xx2(j,lth,lzt),xx3(j,lth,lzt)
		        end do
		    end do
		end do

		write(116,'("POINT_DATA ",i10)') points

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!   8. Write vector data 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

		write(116,'("VECTORS Vvec float")')

		do j=0,mj_r
                    do lth = 0,lth_r
                        do lzt = 0,lzt_r
			write(116,'(1pe15.8," ",1pe15.8," ",1pe15.8)') Vr_b(j,lth,lzt),Vth_b(j,lth,lzt),vthprlf_b(j,lth,lzt)
		        end do
		    end do
		end do
