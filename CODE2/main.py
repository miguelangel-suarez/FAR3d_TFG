...
data_manager = DataManager(csv_path=db_csv, grid_bounds=grid_bounds)
...
print(f"[*] Se van a ejecutar {len(todas_las_combinaciones)} simulaciones.\n")

"""
# Directorio para guardar los logs por si fallan
...
for run_id, combinacion_actual in enumerate(todas_las_combinaciones):
    ...
    manager.modify_input_model('Input_Model', 'Input_Model', current_input_vars, nn_val=nn, mm_vals=mm)
    ...
    if exec_result['status'] == "SUCCESS":
        ...
        if escalares:
            print(
                f"    [+] Datos extraídos para n= {nn}: gam = {escalares.get('gam', 'N/A')}, om_r = {escalares.get('om_r', 'N/A')},"
        ...
        input_summary = {**current_input_vars, **current_data_vars, "n": nn}
