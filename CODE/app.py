"""
---------------- INTERFAZ WEB -------------------
Bloque de Interfaz de Usuario para manejo de la herramienta FAR3d, con sus 3 capas de desarrollo:
Capa Global, Capa Local, y Capa de Simulaciones Guardadas
...
BASE_DATOS = "./BaseDeDatos/data_final.csv"
CSV_CONTINUO = "./espectros_continuos_Alfven/df_continuo.csv"


# --- CONFIGURACIÓN DE LA PÁGINA Y ESTADO ---
...
df = load_data()
excluidas = ['gam', 'om_r', 'gam_var', 'om_r_var', "tipo_inestabilidad", "modo_m", "pos_radial", "diff_m"]
...
if st.session_state.capa_actual == "Global":
    ...
    target_map = {
        ...
        "Tipo de Inestabilidad AE": {"mean": "tipo_inestabilidad", "var": None, "cmap": "gnuplot2", "edge": "white"}
    }
    ...
    show_variance = st.sidebar.checkbox("Mostrar Varianza (Burbujas)", value=False)
    ...
    if df_filt.empty or var_x == var_y:
        ...
        if t_var is None:
            # Mostrar colores de Estable / No Estable
            diccionario_colores = {
                'Estable/Ruido': '#70d696',
                'BAE': '#d86eb1',
                'TAE': '#d86eb1',
                'EAE': '#d86eb1',
                'NAE': '#d86eb1',
            ...
            categorias = sorted(df_filt[t_mean].dropna().unique())
            mapping = {cat: i for i, cat in enumerate(categorias)}
            pivot_cat = df_filt.pivot_table(index=var_y, columns=var_x, values=t_mean, aggfunc='first')
            pivot_numerico = pivot_cat.replace(mapping)
            colores_presentes = [diccionario_colores.get(cat, '#cccccc') for cat in categorias]
            t_cmap_discreto = ListedColormap(colores_presentes)

            fig, ax = plt.subplots(figsize=(10, 6))

            # Dibujar pcolormesh con vmin y vmax centrados matemáticamente
            im = ax.pcolormesh(pivot_numerico.columns, pivot_numerico.index, pivot_numerico.values,
                               shading='nearest', cmap=t_cmap_discreto,
                               vmin=-0.5, vmax=len(categorias) - 0.5)

            # Ajustar la barra de color para mostrar los textos en lugar de números
            cbar = fig.colorbar(im, ax=ax, label=target_label)
            cbar.set_ticks(range(len(categorias)))
...
# =====================================================================
    ...
    with st.form("form_inputs_locales"):
        ...
        with col_m:
            # Multiselector para mm
            mm_elegidos = st.multiselect("Números Poloidales (m)",
                                         options=list(range(1, 26)),  # De 1 a 25 como opciones disponibles
    ...
    if lanzar_simulacion:
        with st.spinner('Ejecutando simulación de FAR3d y Analizando Física...'):
            ...
            # 2. Inyectar el etiquetado automático
